...
./issue_fairy.py --owner FFmpeg --repo FFmpeg --gcli-account ff --issue-label 'repro/yes,repro/no,repro/no(env),repro/flaky,needs info,needs sample,bug,enhancement,regression,resolution/duplicate,resolution/invalid,resolution/external,resolution/fixed' --llm-review-cmd './pr_review_wrapper.py --podman-gpu nvidia.com/gpu=0 --repo-root ffmpeg --extra-repo-root all_ffmpeg --triage-model openai:gpt-5.6-luna --triage-service-tier flex --model openai:gpt-5.6 --reasoning-effort high --service-tier flex --use-vector-store-search --verbose --debug-response-dir openaidebug --use-web-search --max-tool-calls 100 ' --llm-parallelism 3 --verbose 2 $*
