...
# MIN_AGE_DAYS PROD_CACHE PROD_ISSUE_CACHE PROD_WORKSET TRIAGE_LABELS
# ISSUE_LABELS. Throwaway tooling: delete after the cutover proves out.
...
TRIAGE_LABELS=${TRIAGE_LABELS:-important,enhancement,fix/bug,fix/regression,resolution/invalid,API,API major,needs sample,needs docs,needs testing,resolution/duplicate}
...
(cd "$OLD_TREE" && AB_PAYLOAD_DIR="$AB_DIR/payloads-a" ./fairy.py \
    ...
    --verbose) 2>&1 | tee "$AB_DIR/logA-pr.txt"
...
(cd "$OLD_TREE" && AB_PAYLOAD_DIR="$AB_DIR/payloads-a" ./issue_fairy.py \
    ...
    --llm-review-cmd "$STUB" \
    --verbose) 2>&1 | tee "$AB_DIR/logA-issue.txt"
...
(cd "$NEW_TREE" && AB_PAYLOAD_DIR="$AB_DIR/payloads-b" ./agent.py \
    ...
    --issue-args "--owner $OWNER --repo $REPO --gcli-account $ACCOUNT
        ...
        --issue-label '$ISSUE_LABELS'
        --llm-review-cmd '$STUB'
        --verbose") 2>&1 | tee "$AB_DIR/logB.txt"
