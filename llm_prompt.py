...
Holds the developer prompt (``generate_llm_prompt``: one text in reading
order whose optional passages are multiplied by their condition) plus the
user-message builders (``make_user_text`` and friends) so every provider
wrapper gets its text from here without duplicating it, and the standard
...
TASK_OF_ROLE = {
    ...
    "code_review":        "reviewing a pull request",
...
class PromptFor:
    """Whom a prompt addresses."""
    role: str    # a TASK_OF_ROLE key
    model: str
...
def _machines_text(machines: Sequence[ShellHostSpec]) -> str:
    ...
    return (
        ...
        + bool(arm) * f" Choose {arm} if you want to test arm/arm64 or NEON!"
        + bool(x86) * (
            f" Choose {x86} if you want to test on x86/x86-64, MMX, SSE, AVX,"
            " or a GPU, Vulkan, CUDA, Nvidia!"
        )
        + "\n"
...
# emitted (see the ``## Labels`` section of ``generate_llm_prompt``);
# advertising a label the allowlist forbids made the model reason itself
# into it and then spill that reasoning onto an allowed neighbour (e.g. a
# "needs testing" gap tagged as "needs docs").
...
def generate_llm_prompt(
    *,
    ...
    The prompt is one text in reading order; each optional passage is
    multiplied by the condition that includes it. ``vendor`` is accepted
    and recorded in the signature so future wrappers can plumb it
    through; no per-vendor branching exists yet and none should be added
    without a concrete second consumer to pin against. ``project_facts``
    is the deployment's project-facts prompt section (see
    ``load_project_facts``); the prompt text here is project-neutral.
    """
    del vendor  # reserved; see docstring
    if role not in TASK_OF_ROLE:
    ...
    source_bundle       = "source_bundle"       in features and pr and verdict
    vector_store_search = "vector_store_search" in features
    web_search          = "web_search"          in features
    code_interpreter    = "code_interpreter"    in features
    podman_shell        = "podman_shell"        in features
    repo_names = [root.name for root in repo_roots]
    has_spec_repo    = any(name in ("for_ffmpeg",  "all_ffmpeg") for name in repo_names) and vector_store_search
    has_forgejo_repo = any(name in ("forgejo_git", "all_ffmpeg") for name in repo_names) and vector_store_search
...
- Make sure the messages are worded in a friendly tone and do not read offensive to senior developers. Include "{prefix}{label}" toward the beginning of the message. Do not imply that you will not find more issues in a future review.
...
f"""{f'''##In your Code Reviewer role
...
''' * ctx.reviews_code}\
{f'''##In your Design Reviewer role
...
''' * ctx.reviews_design}\
{f'''##Combiner task
The user message contains independent draft reviews of this {subject_long},
...
- Classify the {subject_long} with the same classes and rules as a normal
...
- After your LLM-{label} identification, put a Scope block.
...
- Carry through help a draft provides beyond issues: helpful replies,
  answers, questions to the {subject_long} author, and process clarifications.
- Provide a 1 paragraph justification of your classification of this {subject_long};
...
''' * ctx.draft}\
##Attached context and tools:
{"The commit(s) and metadata are attached" if pr else "The issue metadata is attached"}
...
{f"A FATE sample-suite snapshot is at {CONTAINER_FATE_SUITE}; run fate tests with ``make fate-<name> SAMPLES={CONTAINER_FATE_SUITE}`` and refresh a stale sample with ``make fate-rsync SAMPLES={CONTAINER_FATE_SUITE}`` when needed.\n" * ("ffmpeg" in mount_names)}\
{_machines_text(machines)}

''' if podman_shell and container_repo_mounts else ""}\
{"The **shell** function tool runs shell commands in an ephemeral Linux environment with internet access.\n\n" * (podman_shell and not container_repo_mounts)}\
...
f"""{'''## CI failure mode (this request)
...
''' * (triager and bool(allowed_models))}\
{f"If the community explicitly asks for a more or less verbose or\ndetailed review, set ``requested_verbosity`` ({', '.join(VERBOSITY_LEVELS)}).\n" * triager}\
{f'''## Labels
Allowed labels: {", ".join(allowed_labels)}.
Use ``label_changes`` to add or remove labels when the correct set of labels differs from the current. Each entry is an object with ``label`` (an allowed name), ``op`` (``add`` or ``remove``), ``reason`` (one concrete sentence justifying the change; if you cannot justify it, omit the change), and ``post``. When in doubt about a label, neither add nor remove it. The list may be empty.
Set ``post`` to true only when the reason is needed for a reader to understand why the label is there and should be posted as a comment; set it to false when the reason only serves logs.
{"".join(f"Label: {name}, {meaning}\n" for name, meaning in TRIAGE_LABEL_DEFINITIONS.items() if name in allowed_labels)}\
''' * bool(allowed_labels)}\
...
- Check requests: Have you identified all open requests and open problems related to this {subject_long} and attempted to help?
...
''' * ctx.draft}\
{f'''Message Rules:
- message {"may be empty only for approve and skip" if ctx.classifies else "must not be empty"}.
