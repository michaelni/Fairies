...
``recollq -n 20 <terms>`` searches a prebuilt full-text index of the checkouts{". Like the vector stores, it is mainly for finding material in the pull requests & issues (e.g. ``recollq lowres dir:/work/all_ffmpeg/forgejo_git``) and the multimedia specification documents" if "all_ffmpeg" in mount_names else ""}. The index is refreshed about daily: it does not reflect anything you check out, create or modify (use rg for that), and it matches whole words and quoted "phrases", not regexes or substrings.
